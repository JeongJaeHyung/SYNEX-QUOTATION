...
async def fetch_parts_index(client: httpx.AsyncClient) -> dict:
    ...
    skip, limit = 0, 1000
    while True:
        resp = await client.get(PARTS_LIST_URL, params={"skip": skip, "limit": limit}, timeout=30.0)
        data = resp.json()
        items = data.get("items", [])
        for item in items:
            m_name, p_name = clean_maker_name(item.get("maker_name")), clean_value(item.get("name"))
            major, minor = clean_value(item.get("major_category")), clean_value(item.get("minor_category"))
            payload = {"maker_id": item["maker_id"], "resources_id": item["id"], "solo_price": item.get("solo_price", 0)}
            
            # 매칭 키 생성
            m_k, p_k = normalize_key(m_name), normalize_key(p_name)
            if m_k and p_k: index["by_maker_model"][(m_k, p_k)] = payload
            if not p_k and major and minor: index["by_mm_empty_name"][(normalize_key(major), normalize_key(minor))] = payload
        
        skip += limit
        if skip >= data.get("total", 0): break
    return index
...
async def ensure_part(client: httpx.AsyncClient, row_data: dict, index: dict):
    ...
    if not found and major and minor and not p_k:
        found = index["by_mm_empty_name"].get((normalize_key(major), normalize_key(minor)))
    
    if found or not AUTO_CREATE_MISSING_PARTS: return found
    ...
    async with WRITE_SEMAPHORE:
        ...
        if 200 <= resp.status_code < 300:
            new_p = resp.json()
            res = {"maker_id": new_p["maker_id"], "resources_id": new_p["id"], "solo_price": new_p.get("solo_price", 0)}
            if m_k and p_k: index["by_maker_model"][(m_k, p_k)] = res
            return res
    return None
...
async def process_sheet(client: httpx.AsyncClient, ws, parts_index: dict):
    ...
    header = [str(c.value).strip() if c.value else "" for v in ws.iter_rows(min_row=1, max_row=1) for c in v]
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        if not name and major not in (SUMMARY_MAJORS | LABOR_MAJORS): continue
        if name == "-": continue
        ...
        row_info = {
            ...
            "unit": clean_value(row[h_map["단위"]]) or ("M/D" if major in LABOR_MAJORS else "ea"),
            "solo_price": parse_int(row[h_map["단가"]]),
            "quantity": parse_int(row[h_map["수량"]])
        }
        
        if row_info["quantity"] <= 0 and major not in LABOR_MAJORS: continue
    ...
    payload = {"name": machine_name, "creator": TEMPLATE_CREATOR, "resources": resources_payload}
    
    async with WRITE_SEMAPHORE:
        # 기존 검색
        search_resp = await client.get(MACHINE_SEARCH_URL, params={"search": machine_name})
        items = search_resp.json().get("items", [])
        existing = next((i for i in items if i["name"] == machine_name), None)
        
        if existing and UPSERT_TEMPLATES:
            r = await client.put(f"{MACHINE_API_URL}{existing['id']}", json=payload)
            print(f"✅ 갱신 완료: {machine_name}")
        else:
            r = await client.post(MACHINE_API_URL, json=payload)
            print(f"✅ 신규 등록: {machine_name}")

async def main():
    if not os.path.exists(EXCEL_FILE_PATH): return print("❌ 엑셀 파일 없음")
    wb = load_workbook(EXCEL_FILE_PATH, data_only=True)
    
    async with httpx.AsyncClient(timeout=60.0) as client:
        parts_index = await fetch_parts_index(client)
        # 시트별로 비동기 작업 생성 (단, 내부 쓰기 작업은 세마포어로 보호됨)
        tasks = [process_sheet(client, wb.worksheets[i], parts_index) for i in TEMPLATE_SHEET_INDICES if i < len(wb.worksheets)]
