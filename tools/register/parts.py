...
def get_parts_data_from_excel(path, sheet_idx):
    ...
    header_row_index = 1
    header = [str(cell.value).strip() if cell.value is not None else "" for cell in ws[header_row_index]]
    ...
    for row in ws.iter_rows(min_row=header_row_index + 1, values_only=True):
        raw_part_name = str(row[h_map['모델명/규격']] if h_map['모델명/규격'] < len(row) else "").strip()
        if raw_part_name == '-':
            skipped_count += 1
            continue

        try:
            price_str = str(row[h_map['단가']] if h_map['단가'] < len(row) else "0").replace(',', '').strip()
            solo_price = int(float(price_str)) if price_str and price_str != "None" else 0
...
async def post_part(client, sem, data, index):
    ...
    async with sem:
        try:
            ...
            if 200 <= r.status_code < 300: return "OK"
            if r.status_code == 409: return "EXIST"
...
async def main():
    async with httpx.AsyncClient() as client:
        # 1. 엑셀 데이터 파싱
        print("📊 엑셀 파일 읽는 중...")
        excel_parts, skipped = get_parts_data_from_excel(EXCEL_FILE_PATH, PARTS_SHEET_NAME)
        if not excel_parts: return print("❌ 등록할 부품이 없습니다.")
        ...
        print(f"🚀 총 {len(excel_parts)}개 중 {len(new_parts)}개 신규 등록 시작 (중복 제외: {len(excel_parts) - len(new_parts)}개)...")
