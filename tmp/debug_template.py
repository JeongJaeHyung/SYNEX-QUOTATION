...
def debug_sheet_raw_via_api():
    ...
    print("Row 4: [...] (생략)")
    print("Row 5: ['[0]구분', '[1]품명', '[2]단위', '[3]수량', '[4]단가', '[5]금액'] (가정된 컬럼 헤더)")
    ...
    for i, mr in enumerate(labor_resources, base_row_index):
        ...
        row_data = {
            ...
            1: part.get('name', 'N/A') or mr.get('display_model_name', 'N/A'), # 품명 (Master Name 또는 Display Name)
        ...
        if not part:
            print(f"  [⚠️ WARNING]: Resource ID {mr.get('resources_id')}의 마스터 데이터가 API 응답에 누락되었습니다.")
