...
def create_header(
    ...
    # 3. 데이터 그룹화
    machine_groups = defaultdict(lambda: {"자재비": 0, "인건비": 0})  # machine별 자재비/인건비
    ...
    for resource in detailed_resources:
        ...
        if major_normalized == "자재비":
            ...
            machine_groups[resource.machine_name]["자재비"] += resource.solo_price * resource.compare
        
        elif major_normalized == "인건비":
            machines.add(resource.machine_name)
            machine_groups[resource.machine_name]["인건비"] += resource.solo_price * resource.compare
    ...
    for machine in sorted(machines):
        # 재료비 ("자재비" → "재료비")
        header_resources_list.append(HeaderResources(
            header_id=header.id,
            machine=machine,
            name="재료비",
            solo_price=machine_groups[machine]["자재비"],
            compare=1, unit="원", spac="", description=""
        ))
...
def get_header_by_id(db: Session, header_id: UUID) -> Optional[Header]:
    """Header 단일 조회"""
...
def get_header_resources(db: Session, header_id: UUID) -> List[dict]:
    ...
    """
    resources = db.query(HeaderResources).filter(HeaderResources.header_id == header_id).all()
    result = []
    for r in resources:
        result.append({
            "machine": r.machine,
...
def update_header(
    ...
    header_resources: Optional[List[dict]] = None
) -> Optional[Header]:
    """
    Header 수정 (부분 수정)
    ...
    if header_resources is not None:
        # 기존 resources 삭제
        db.query(HeaderResources).filter(HeaderResources.header_id == header_id).delete()
        db.flush()  # ✅ 삭제를 즉시 반영!
