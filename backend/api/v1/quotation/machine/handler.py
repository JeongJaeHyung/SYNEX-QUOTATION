...
from backend.api.v1.quotation.machine import (
    crud,
    schemas,
)  # Machine 스키마(DTO) 및 CRUD 함수 임포트
from backend.database import get_db  # 데이터베이스 세션 의존성 주입
from backend.models.resources import (
    Resources,
)  # Resources 모델 임포트 (자재 존재 여부 검증용)
...
def get_machine_list_schema() -> dict:
    ...
    """
    return {
        "name": {"title": "장비명", "type": "string", "ratio": 4, "align": "left"},
        "manufacturer": {
        ...
        "client": {"title": "고객사", "type": "string", "ratio": 2, "align": "center"},
        "creator": {"title": "작성자", "type": "string", "ratio": 1, "align": "center"},
        "updated_at": {
            "title": "최종수정일",
            "type": "datetime",
        ...
        "description": {"title": "비고", "type": "string", "ratio": 3, "align": "left"},
    }
...
def get_machine_resources_schema() -> dict:
    ...
    """
    return {
        "item_code": {"title": "품목코드", "type": "string", "ratio": 2},
        "maker_name": {"title": "Maker", "type": "string", "ratio": 2},
        "category_major": {"title": "Unit", "type": "string", "ratio": 2},
        "category_minor": {"title": "품목", "type": "string", "ratio": 2},
        "model_name": {"title": "모델명/규격", "type": "string", "ratio": 3},
        ...
        "quantity": {"title": "수량", "type": "integer", "ratio": 1},
        "subtotal": {
            "title": "합계 금액",
            "type": "integer",
            "format": "currency",
...
def register_machine(
    ...
    for resource in machine.resources:
        ...
        if resource.maker_id in [
            "SUMMARY",
            "LABOR",
            "T000",
        ]:  # 'T000'도 특수 목적으로 사용될 수 있으므로 포함
            continue
...
@handler.get(
    ...
    response_model=Union[
        schemas.MachineListWithSchemaResponse, schemas.MachineListResponse
    ],
)
def get_machines(
...
def get_machine(
    ...
    """
    machine = crud.get_machine_by_id(
        db, machine_id
    )  # CRUD 함수를 통해 Machine 조회 (MachineResources 함께 로드)
    if not machine:
