...
from sqlalchemy import desc, func  # SQLAlchemy 함수 (예: count, 내림차순 정렬) 임포트
from sqlalchemy.orm import (
    Session,
    joinedload,
)  # 데이터베이스 세션 및 관계된 객체 미리 로드

from backend.models.certification import (
    Certification,
)  # Certification 모델 임포트 (인증 정보)
from backend.models.machine import Machine  # Machine 모델 임포트
from backend.models.machine_resources import (
    MachineResources,
)  # MachineResources 모델 임포트
...
def create_machine(
    db: Session,
    name: str,  # 장비/견적서 이름
    manufacturer: str | None,  # 장비사
    client: str | None,  # 고객사
    creator: str,  # 작성자
    description: str | None,  # 비고
...
def search_machines(
    ...
    machines = query.order_by(desc(Machine.updated_at)).offset(skip).limit(limit).all()

    return total, machines


def get_machine_by_id(db: Session, machine_id: UUID) -> Machine | None:
    """
    ...
    return (
        db.query(Machine)
        .options(
            joinedload(Machine.machine_resources)
        )  # MachineResources 관계를 Eager Loading
...
def get_machine_resources_detail(db: Session, machine_id: UUID) -> list[dict]:
    ...
    for mr in machine_resources:
        ...
        if (
            mr.maker_id == "SUMMARY" or mr.maker_id == "T000"
        ):  # T000도 SUMMARY처럼 처리될 수 있음 (관례)
            item_name_map = {
                "LOCAL_MAT": "Local 자재",
            ...
            resources.append(
                {
                    ...
                    "model_name": mr.display_model_name
                    or item_name_map.get(
                        mr.resources_id, mr.resources_id
                    ),  # 화면 표시용 이름
        ...
        # 인건비 항목 역시 가상으로 데이터를 생성합니다.
            ...
            mr.maker_id == "LABOR" or mr.maker_id == "T000"
        ):  # T000이 LABOR로도 쓰일 수 있음
            # resources_id가 "LABOR_0", "LABOR_1"과 같은 형식일 경우 "인건비 " 접두사를 붙여 이름 생성
            labor_name = mr.display_model_name or mr.resources_id.replace(
                "LABOR_", "인건비 "
            )
        ...
        # 실제 Resources 테이블과 Maker, Certification 테이블을 조인하여 상세 정보를 가져옵니다.
            ...
            result = (
                db.query(
                    ...
                    Resources.major.label(
                        "category_major"
                    ),  # Resources 마스터의 대분류
                    Resources.minor.label(
                        "category_minor"
                    ),  # Resources 마스터의 중분류
                ...
                .join(
                    Resources,
                    (
                        MachineResources.maker_id == Resources.maker_id
                    )  # Resources와 조인
                ...
                .outerjoin(
                    Certification,
                    (
                        Resources.id == Certification.resources_id
                    )  # Certification과 Outer Join (인증 정보가 없을 수도 있음)
                ...
                .filter(
                    MachineResources.maker_id == mr.maker_id
                )  # 현재 처리 중인 MachineResources와 일치하는 자재
...
def update_machine(
    ...
    resources: list[dict] | None = None,  # 업데이트할 자재 목록
) -> Machine | None:
    """
