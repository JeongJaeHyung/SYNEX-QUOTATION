...
def create_price_compare(
    request: schemas.PriceCompareCreate, db: Session = Depends(get_db)
):
    """
    **내정가 비교서 생성**
...
def get_price_compare(price_compare_id: UUID, db: Session = Depends(get_db)):
    """
    **내정가 비교서 상세 조회**
    """
...
@handler.delete(
    "/{price_compare_id}", status_code=status.HTTP_200_OK, summary="내정가 비교서 삭제"
)
def delete_price_compare(price_compare_id: UUID, db: Session = Depends(get_db)):
    """
    **내정가 비교서 삭제**
    - 관련된 PriceCompareMachine, PriceCompareResources도 함께 삭제됩니다.
    """
    success = crud.delete_price_compare(db, price_compare_id)
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Price compare document not found",
    ...
    return {
        "message": "Price compare deleted successfully",
        "deleted_id": str(price_compare_id),
    }
