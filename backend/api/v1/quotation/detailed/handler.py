...
def get_detailed_resources_schema() -> dict:
    ...
    return {
        "machine_name": {"title": "장비명", "type": "string", "ratio": 2},
        "major": {"title": "대분류", "type": "string", "ratio": 2},
        "minor": {"title": "중분류", "type": "string", "ratio": 2},
        ...
        "compare": {"title": "수량", "type": "integer", "ratio": 1},
        "subtotal": {
            "title": "견적가",
            "type": "integer",
            "format": "currency",
...
def create_detailed(
    detailed_data: schemas.DetailedCreate, db: Session = Depends(get_db)
):
    """
    Detailed 등록
    ...
    detailed = crud.create_detailed(
        db=db,
        folder_id=detailed_data.folder_id,
...
def delete_detailed(detailed_id: UUID, db: Session = Depends(get_db)):
    """
    Detailed 삭제
    ...
    return {"message": "Detailed deleted successfully", "deleted_id": str(detailed_id)}
