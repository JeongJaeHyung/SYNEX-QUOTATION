# api/v1/part/crud.py
#
# 부품(Resources) 데이터베이스 작업을 위한 CRUD(Create, Read, Update, Delete) 함수를 정의합니다.  # noqa: E501
# - 부품 ID 생성, 부품 및 인증 정보 생성/조회/검색/수정/삭제 기능을 제공합니다.
#

from typing import List, Optional, Tuple  # 타입 힌트  # noqa: UP035

from sqlalchemy import and_, func, or_  # SQLAlchemy AND, OR 조건 및 함수 사용을 위함
...
def get_maker_by_name(db: Session, maker_name: str) -> Maker | None:
    """
...
def get_next_parts_id(db: Session, maker_id: str) -> str:
    ...
    max_id = (
        db.query(func.max(Resources.id)).filter(Resources.maker_id == maker_id).scalar()
    )
...
def get_parts_by_fields(
    db: Session, major: str, minor: str, name: str
...
def create_parts(
    ...
    unit: str,
    solo_price: int,
    display_order: int | None = None,  # 표시 순서 (지정 없으면 자동 할당)
    ul: bool = False,
    ce: bool = False,
    kc: bool = False,
    etc: str | None = None,
) -> Resources:
    # 💡 [수정] display_order가 None인 경우, DB의 NOT NULL 제약조건을 만족시키기 위해 0을 할당합니다.  # noqa: E501
    if display_order is None:
        display_order = 0

    # Resources 모델 인스턴스 생성
    # 💡 [복구] display_order 인자 전달을 다시 추가합니다. (이제 모델에 정의되어 TypeError가 발생하지 않음)  # noqa: E501
    resource = Resources(
        id=parts_id,
        maker_id=maker_id,
        major=major,
    ...
    certification = Certification(
        resources_id=parts_id, maker_id=maker_id, ul=ul, ce=ce, kc=kc, etc=etc
    )
...
def get_parts_by_id(db: Session, parts_id: str, maker_id: str) -> Resources | None:
    """
    ...
    return (
        ...
        .filter(
            and_(
                Resources.id == parts_id, Resources.maker_id == maker_id
            )  # 복합 PK 조건
        )
        .first()
    )
...
def get_parts_list(
    ...
    """  # noqa: E501
    query = db.query(Resources).join(
        Resources.maker
    )  # Resources와 Maker를 조인하여 제조사 이름 필터링에 대비  # noqa: E501
    ...
    # --- 정렬 및 페이징 적용 ---
    parts_list = (
        query.order_by(Resources.maker_id.asc(), Resources.id.asc())
        .offset(skip)
        .limit(limit)
        .all()
    )  # 품목코드(maker_id + id) 순서로 정렬  # noqa: E501
...
def search_parts(
    ...
    """  # noqa: E501
    query_obj = db.query(Resources).join(
        Resources.maker
    )  # Maker 정보를 함께 검색하기 위해 조인  # noqa: E501
    ...
    parts_list = (
        query_obj.order_by(Resources.maker_id.asc(), Resources.id.asc())
        .offset(skip)
        .limit(limit)
        .all()
    )  # 품목코드 순서로 정렬  # noqa: E501
...
def update_parts(
    ...
    parts_id: str,
    maker_id: str,
    major: str | None = None,  # 대분류
    minor: str | None = None,  # 중분류
    name: str | None = None,  # 모델명/규격
    unit: str | None = None,  # 단위
    solo_price: int | None = None,  # 단가
    ul: bool | None = None,  # UL 인증 여부
    ce: bool | None = None,  # CE 인증 여부
    kc: bool | None = None,  # KC 인증 여부
    etc: str | None = None,  # 기타 인증/비고
) -> Resources | None:
    """
