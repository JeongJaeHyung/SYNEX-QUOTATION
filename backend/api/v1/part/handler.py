...
# - DTO(Pydantic 스키마)와 CRUD(데이터베이스 작업) 계층을 연결하는
#   컨트롤러 역할을 합니다.
#
...
from fastapi import (
    ...
    HTTPException,
    Query,
)  # FastAPI 라우터, 의존성 주입, 쿼리 파라미터, HTTP 예외 처리
...
def get_parts_schema() -> dict:
    ...
    return {
        ...
        "maker_name": {"title": "Maker", "type": "string", "ratio": 2},
        "major_category": {"title": "Unit", "type": "string", "ratio": 4},
        "minor_category": {"title": "품목", "type": "string", "ratio": 3},
        "name": {"title": "모델명/규격", "type": "string", "ratio": 10},
        "unit": {"title": "단위", "type": "string", "ratio": 0.1},
        "solo_price": {
            "title": "금액",
            "type": "integer",
        ...
        "ul": {"title": "UL", "type": "boolean", "ratio": 0.1},
        "ce": {"title": "CE", "type": "boolean", "ratio": 0.1},
        "kc": {"title": "KC", "type": "boolean", "ratio": 0.1},
        "etc": {"title": "기타", "type": "string", "ratio": 3},
    }
...
def convert_to_parts_response(resource) -> dict:
    ...
    return {
        ...
        "display_order": getattr(
            resource, "display_order", None
        ),  # display_order 속성 가져오기
...
def create_parts(
    ...
    try:
        ...
        if not maker:
            raise HTTPException(
                status_code=404,
                detail=f"제조사 '{parts_data.maker_name}'을(를) 찾을 수 없습니다.",
            )

        # --- 중복 검사: major, minor, name ---
        existing = crud.get_parts_by_fields(
            db, parts_data.major_category, parts_data.minor_category, parts_data.name
        )
        if existing:
            item_code = f"{existing.maker_id}-{existing.id}"
            raise HTTPException(
                status_code=409,
                detail=f"중복된 부품이 존재합니다. item_code={item_code}",
            )
        ...
        return convert_to_parts_response(
            resource
        )  # 생성된 자원 객체를 응답 DTO로 변환하여 반환
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"DEBUG: create_parts error: {str(e)}"
        ) from e
...
def get_parts_list(
    ...
    id: str | None = Query(None, description="부품 ID로 필터링"),
    maker_id: str | None = Query(None, description="제조사 ID로 필터링"),
    name: str | None = Query(None, description="모델명/규격으로 필터링"),
    unit: str | None = Query(None, description="단위로 필터링"),
    min_price: int | None = Query(None, ge=0, description="최소 단가"),
    max_price: int | None = Query(None, ge=0, description="최대 단가"),
    major: str | None = Query(None, description="대분류로 필터링"),
    minor: str | None = Query(None, description="중분류로 필터링"),
    ul: bool | None = Query(None, description="UL 인증 여부로 필터링"),
    ce: bool | None = Query(None, description="CE 인증 여부로 필터링"),
    kc: bool | None = Query(None, description="KC 인증 여부로 필터링"),
    db: Session = Depends(get_db),  # DB 세션 의존성 주입
):
    try:
        ...
        msg = f"DEBUG: get_parts_list error: {str(e)}"
...
def get_parts_detail(
    ...
    try:
        parts = crud.get_parts_by_id(
            db, parts_id, maker_id
        )  # CRUD 함수를 통해 부품 조회
        ...
        # 스키마 포함 옵션에 따라 응답을 구성합니다.
        if include_schema:
            return {"schema": get_parts_schema(), "item": item}
        return item
    except HTTPException:
        raise
    except Exception as e:
        msg = f"DEBUG: get_parts_detail error: {str(e)}"
        raise HTTPException(status_code=500, detail=msg) from e


@handler.post("/search")
def search_parts(
    search_request: schemas.PartsSearchRequest,  # 요청 바디는 PartsSearchRequest 따름
    db: Session = Depends(get_db),  # DB 세션 의존성 주입
):
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"DEBUG: search_parts error: {str(e)}"
        ) from e
...
def update_parts(
    ...
    try:
        ...
        return convert_to_parts_response(
            updated_parts
        )  # 업데이트된 부품 객체를 응답 DTO로 변환하여 반환
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"DEBUG: update_parts error: {str(e)}"
        ) from e
...
def delete_parts(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"DEBUG: delete_parts error: {str(e)}"
        ) from e
