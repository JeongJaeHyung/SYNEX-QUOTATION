...
def create_maker(
    ...
    if existing:
        raise HTTPException(status_code=409, detail=f"제조사 '{maker_data.name}'이(가) 이미 존재합니다.")
    
    # --- ID 중복 체크 (ID가 요청에 포함된 경우) ---
    if maker_data.id:
        existing_id = crud.get_maker_by_id(db, maker_data.id)
        if existing_id:
            raise HTTPException(status_code=409, detail=f"제조사 ID '{maker_data.id}'이(가) 이미 존재합니다.")
...
def update_maker(
    ...
    Raises:
        HTTPException: 제조사 ID를 찾지 못한 경우 404 Not Found.
    """
    updated_maker = crud.update_maker(db, maker_id, maker_update.name) # CRUD 함수를 통해 제조사 업데이트
