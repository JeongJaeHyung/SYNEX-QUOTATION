...
def get_next_maker_id(db: Session) -> str:
    ...
    - 형식: J001, J002, J003...
    - 기존 'J'으로 시작하는 ID 중 최대값에 1을 더하여 다음 ID를 생성합니다.

    Args:
        db (Session): SQLAlchemy 데이터베이스 세션.

    Returns:
        str: 새로 생성된 4자리 제조사 ID (예: "J001").
    """
    # 'J'으로 시작하는 기존 Maker ID 중 최대값을 조회합니다.
    max_id = db.query(func.max(Maker.id)).filter(Maker.id.like("J%")).scalar()

    if not max_id:
        return "J001"  # 기존 ID가 없으면 "J001" 반환

    # 최대 ID의 숫자 부분만 추출하여 1을 더하고, 다시 3자리 숫자로 포맷팅합니다.
    try:
        num = int(max_id[1:]) + 1  # "J001" -> 1 -> 2
        return f"J{num:03d}"  # 2 -> "J002"
    except (ValueError, TypeError):  # ID가 숫자로 변환 불가능한 경우 (예외 처리)
        return "J001"  # 오류 발생 시 기본값 반환
...
def create_maker(
    db: Session,
    name: str,  # 제조사 이름
    maker_id: str | None = None,  # ID가 제공되지 않으면 자동 생성
) -> Maker:
    """
...
def get_maker_by_id(db: Session, maker_id: str) -> Maker | None:
    """
...
def get_maker_by_name(db: Session, name: str) -> Maker | None:
    """
...
def search_makers(
    ...
    """
    query_obj = db.query(Maker).filter(
        Maker.name.ilike(f"%{query}%")
    )  # 이름 부분 매칭 (대소문자 구분 없음)
    total = query_obj.count()  # 검색 결과 총 개수
    makers = (
        query_obj.order_by(Maker.created_at.desc())  # 생성일시 기준으로 내림차순 정렬
        .offset(skip)
        .limit(limit)
        .all()
    )
