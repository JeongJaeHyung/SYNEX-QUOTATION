...
def group_by_major_then_machine(resources: List[Dict[str, Any]]) -> Dict[str, Dict[str, List]]:
    """
...
def create_excel(data: Dict[str, Any]) -> BytesIO:
    ...
    # 배경색
    fill_header = PatternFill(start_color="DBEAFE", end_color="DBEAFE", fill_type="solid")  # 하늘색
    fill_subtotal = PatternFill(start_color="FEF9C3", end_color="FEF9C3", fill_type="solid")  # 노란색
    fill_total = PatternFill(start_color="FDE68A", end_color="FDE68A", fill_type="solid")  # 진한 노란색
    fill_note_head = PatternFill(start_color="F2F2F2", end_color="F2F2F2", fill_type="solid")  # 회색
    ...
    for cell_addr, value, span in headers_main:
        cell = ws[cell_addr]
        cell.value = value
        cell.font = font_bold
    ...
    sorted_majors = sorted(
        groups.keys(),
        key=lambda x: major_order.index(x) if x in major_order else 99
    )
    ...
    for major in sorted_majors:
        ...
        for machine_name, items in machine_groups.items():
            ...
            for idx, item in enumerate(items):
                # 금액 계산
                cost_amt = (item.get('cost_compare', 0) or 0) * (item.get('cost_solo_price', 0) or 0)
                quote_amt = (item.get('quotation_compare', 0) or 0) * (item.get('quotation_solo_price', 0) or 0)
                major_cost += cost_amt
