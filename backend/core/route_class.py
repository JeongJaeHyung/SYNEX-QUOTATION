...
from models import role_permission, Permission, Account
...
class RBACRoute(APIRoute):
    def get_route_handler(self) -> Callable:
        ...
        async def custom_route_handler(request: Request):
            ...
            try:
                ...
                if resource == "unknown":
                    print(f"[Security Warning] Unknown Resource Access Attempt: {request.url.path}")
                    raise HTTPException(status_code=403, detail="접근 권한이 없는 리소스입니다.")
                ...
                has_permission = (
                    ...
                    .join(Permission, role_permission.c.permission_id == Permission.id) # [핵심 수정] .c 사용
                    .filter(
                        role_permission.c.role_id == user.role_id,  # [핵심 수정] .c 사용
                ...
                if not has_permission:
                    print(f"[Access Denied] User: {user.id}, Role: {user.role_id}, Target: {resource}:{action}")
    ...
    def _detect_resource(self, path: str) -> str:
        ...
        if "quotation" in parts:
            if "machine" in parts: return "machine"
            if "general" in parts: return "general"
            return "quotation" # 견적서 공통
            
        # 예: /api/v1/parts -> parts
        if "parts" in parts: return "parts"
        if "maker" in parts: return "maker"
        if "account" in parts: return "account"
